...
# 2. Generate an enhanced map visualization for a specific tile
python main.py map 4815081 --max-pois 500 --cluster-distance 30

# 3. Create a multi-tile visualization (combines multiple maps)
python multi_tile_viz.py 4815075 4815081 4815428 --name my_area

# 4. Validate and correct POI295 violations
python main.py validate --correct --report --visualize

# 5. View the generated maps and reports in the data/processed/ folder
```
...
- **Color coding**: Each color represents a different POI facility type, with similar types sharing similar colors
...
- **Clustering**: When using `--max-pois` and `--cluster-distance` parameters, POIs are intelligently clustered to prevent overcrowding
- **Multi-tile visualization**: The `multi_tile_viz.py` tool allows combining multiple tile visualizations into a single comprehensive view

#### Visualization Improvements

The enhanced visualization includes:

- **Smart POI clustering**: Automatically groups nearby POIs of similar types to avoid visual clutter
- **Better color organization**: POI types are colored by category for easier identification
- **Improved layout**: Cleaner design with better information layout and summary statistics
- **Multi-tile support**: Create combined visualizations of multiple areas with `multi_tile_viz.py`

When satellite imagery is available (with a valid HERE API key), POIs are overlaid on the satellite map. Without satellite imagery, an enhanced styled map is generated with a clean modern design.
...
The project includes several tools to create enhanced visualizations of the POI data:

```bash
# Generate an enhanced map with clustering
python main.py map 4815075 --max-pois 500 --cluster-distance 30

# Create a multi-tile visualization combining several areas
python multi_tile_viz.py 4815075 4815087 4815425 4815428 --name guadalajara_region

# Compare original and improved visualizations
python comparison.py original_file.png improved_file.png comparison_result
```

These tools help identify patterns across different tiles and provide a more comprehensive view of the POI data.
